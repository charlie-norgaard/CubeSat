...
module Cosmos
  ...
  class AckProtocol < Protocol
    ...
    def read_data(data)
      ...
      packet_id = BinaryAccessor.read(32, 32, :UINT, data, @endianness)
      
      # NEED TO CHANGE SO THAT PACKET ID IS LOGGED
      if calculated_crc != crc
        Logger.error "#{@interface ? @interface.name : ""}: Invalid CRC detected! Calculated 0x#{calculated_crc.to_s(16).upcase} vs found 0x#{crc.to_s(16).upcase}. PKT_ID = #{packet_id}."
        Logger.info "#{@interface ? @interface.name : ""}: NACK Flag Raised! Packet with PKT_ID = #{packet_id} has been queued for a resend request."

        # Need to log packet ID somewhere that is accessible by a script
    ...
    def write_packet(packet)
      if @write_item_name
        ...
        crc = @crc.calc(packet.buffer(false)[0...end_range])
                          Logger.info "#{@interface ? @interface.name : ""}: Outgoing CRC: #{crc}."
